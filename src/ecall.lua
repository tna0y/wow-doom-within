function _DW_HandleEcall(game)
    return function(CPU, syscall_num)
        ...
        elseif syscall_num == 109 then -- draw_pixel
            local x = registers[10]
            local y = registers[11]
            local color = registers[12]
            local framebuffer = game.framebuffer
            framebuffer[x + y * 320] = color
        elseif syscall_num == 110 then -- draw_rect
            local xmin = registers[10]
            local xmax = registers[11]
            local ymin = registers[12]
            local ymax = registers[13]
            local color = registers[14]
            local framebuffer = game.framebuffer
            for y = ymin, ymax do
                for x = xmin, xmax do
                    framebuffer[x + y * 320] = color
        ...
        elseif syscall_num == 111 then -- memcpy
