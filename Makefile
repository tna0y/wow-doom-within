...
AOT2LUA = ./risc-v-wow-emu/tools/aot_compile.py
AOT2LUA_TEMPLATE = ./risc-v-wow-emu/tools/aot.lua.j2
...
doomgeneric_aot: doomgeneric
	cat $(OUTPUT) | $(PYTHON) $(AOT2LUA) doom $(AOT2LUA_TEMPLATE) > $(BUILD_BIN_DIR)/doomgeneric_aot.lua

addon: doomgeneric_lua doomgeneric_aot
	...
	cp $(BUILD_BIN_DIR)/doomgeneric_aot.lua $(ADDON_BUILD_DIR)/doomgeneric_aot.lua
...
.PHONY: doomgeneric doomgeneric_lua doomgeneric_aot addon release clean
